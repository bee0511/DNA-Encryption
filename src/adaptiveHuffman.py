        ...
        with open(input, "rb") as fin:
            with open(output, "wb") as fout:
                ...
                while byte:
                    progress.update(1)
                    bits = "{0:b}".format(ord(byte)).zfill(8)
                    buffer.extend(self._get_code(bits))
                    self._update_tree(bits)
                    byte = fin.read(1)

                    while len(buffer) >= 8:
                        fout.write(bytes([int(''.join(buffer[:8]), 2)]))
                        buffer = buffer[8:]
                if buffer:
                    buffer.extend(self._get_code("NYT"))
                    buffer.extend("00000000")  # padding
